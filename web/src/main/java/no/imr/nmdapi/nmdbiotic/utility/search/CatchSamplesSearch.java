...
public class CatchSamplesSearch implements IBioticFilter {
    ...
    private int findPosition(int searchedPosition, int size) {
        ...
        //return isLegalPosition(pos, size) ? pos : pos >= size ? size-1 : 0; 
    ...
    private int toPosition(int fromPosition, BigInteger value, BigInteger prevValue, int size) {
        //int toPos = fromPosition(searchedPosition, size);
    ...
    private Range getRangeFiles(
            List<SerialNumberIndexedData> serialNumberList
          , BigInteger fromSerialNo
          , BigInteger toSerialNo
          , Map<File, BigInteger> fileMap
          ) {
      Range range = null;
    ...
    public String find(
          ...
          , BigInteger fromSerialNo
          , BigInteger toSerialNo
          , Map<String, List<SerialNumberIndexedData>> serialNumberMap
          , Map<String, Integer> specieMap
          , Map<File, String> fileHeaderMap
              ...
              if (range != null) {
                  ...
                  for (Map.Entry<File, BigInteger> entry : fileMap.entrySet()) {
                      ...
                      for (int index = range.getFromInclusivePosition(); index < (range.getToInclusivePosition()+1); index++) {
                          SerialNumberIndexedData element = serialNumberList.get(index);
                          //logger.info(++scounter+"      serialNumberList size ++++++ "+serialNumberList.size());
                          
                          File file = element.getFile();
                          if ((file.equals(fileKey)) && (search.accept(element, specieIndex))) {
                              //long rtime = CommonUtil.printTimeElapsed(0, "start decompressing &&&&&&&&", "");
      ...
      //logger.info("fromSerialNo "+fromSerialNo+" toSerialNo "+toSerialNo+ " size "+result.size() +" found "+(result.size() > 0)) ;

      //return XmlFormatterZip.prettyFormat(sb.toString(), 4);
